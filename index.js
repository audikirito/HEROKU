...
const { meguminmenu } = require('./X1/meguminmenu')
...
kiji = fs.readFileSync('./kiji/kiji.m4a')
		    ...
		    break				    				 
		    case 'meguminmenu':		    
		    helpzzz02 = fs.readFileSync(`./X3/help.jpeg`) 
            const helpzzz03 = await nayla.prepareMessage(from, helpzzz02, MessageType.image, {thumbnail:Buffer.alloc(0)}) 
            const helpzzz04 = [{buttonId: 'id1', buttonText: {displayText: 'DONASI'}, type: 1},{buttonId: 'id2', buttonText: {displayText: 'INFOBOT'}, type: 1},{buttonId: 'id3', buttonText: {displayText: 'OWNERBOT'}, type: 1}]                          
            const helpzzz05 = {            
            contentText: meguminmenu(tz, nama, namabot, prefix, day, yy, thisDay, bulan, wib, wita, wit),
            footerText: `${nama}\n ${tz} RUNTIME : ${kyun(runtime)}`,
            buttons: helpzzz04,
            headerType: 4,
            thumbnail:Buffer.alloc(0),
            imageMessage: helpzzz03.message.imageMessage
            }
            nayla.sendMessage(from, helpzzz05, MessageType.buttonsMessage, {quoted: nay1})            
		    break				    		 
		    case 'gamemenu':		    
            ...
            case 'megumin2':
			try {
        megumin2 = await getBuffer('https://api.lolhuman.xyz/api/random/megumin?apikey=Chikey')
            sendImage(megumin2, "NIHH KAK")
      } catch (e) {
        reply("ApiKey Tidak Valid")
            ...
            case 'ytmp3':
                reply(mess.wait)
                ini_url = args[0]
                get_result = await fetchJson(`http://hadi-api.herokuapp.com/api/yt2/audio?url=${ini_url}`)
                get_result = get_result.result
                get_judul = get_result.title
                get_img = get_result.thumb
                get_size = get_result.size
                get_ext = get_result.ext
                buffer = await getBuffer(get_result.thumb)
                caf = `┏━━⬣ 𝙇𝘼𝙂𝙐 𝘿𝙄𝙏𝙀𝙈𝙐𝙆𝘼𝙉
┃ ❑ *JUDUL :* ${get_result.title}
┃ ❑ *UKURAN :* ${get_result.size}
┃ ❑ *EXTENSION :* ${get_result.ext}
┗━━⬣`
                nayla.sendMessage(from, buffer, image, {quoted: nay1, caption: caf})
                get_url = get_result.download_audio
                lagu = await getBuffer(get_result.download_audio)
                sendAudio(lagu)
                break
            case 'ytmp4':
                reply(mess.wait)
                ...
                get_result = get_result.result
                get_caf = await fetchJson(`http://hadi-api.herokuapp.com/api/ytvideo?url=${ini_url}`)
                get_caf = get_caf.result
                get_judul = get_result.title
                get_desk = get_caf.desc
                get_viewer = get_caf.viewer
                get_like = get_caf.like
                get_img = get_result.thumb
                get_size = get_result.size
                get_resolusi = get_result.resolution
                get_ext = get_result.ext
                buffer = await getBuffer(get_result.thumb)
                caf = `━━━⬣ 𝙑𝙄𝘿𝙀𝙊 𝘿𝙄𝙏𝙀𝙈𝙐𝙆𝘼𝙉
 ❑ *JUDUL :* ${get_result.title}
 ❑ *DESKRIPSI :* ${get_caf.desc}
 ❑ *VIEWER :* ${get_caf.viewer}
 ❑ *LIKE :* ${get_caf.like}
 ❑ *UKURAN :* ${get_result.size}
 ❑ *RESOLUSI :* ${get_result.resolution}
 ❑ *EXTENSION :* ${get_result.ext}
━━━⬣`
                nayla.sendMessage(from, buffer, image, {quoted: nay1, caption: caf})
                get_url = get_result.download_video
